...
int main()   //  it is the main function where the execution begins
{
	/* 	Declaring of variables i,j for iteration purpose,
		size to read the input from user and to define the size of array,temp and an arr to store elements. 
	*/
	...
	for(i = 1;i < size; i++)
	{
		...
		*/
		...
		{
			arr[j+1]=arr[j];   // move temp to the next position
			j = j-1;	// decrementing the positin of j 
		}
		arr[j+1] = temp;    // temp is inserted in the appropriate position as needed to be attained 
	}
	// diplay title of the result
	printf("After sorting: ");
	for(i = 0;i < size; i++)   //  iterating the output as entire elements of the array need to be displayed 
	{
		printf(" %d", arr[i]); //  showing the sorted result
	}
